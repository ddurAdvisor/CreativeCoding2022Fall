class Slider {
  ...
  PImage[] sliderImgs;
  PImage base, bar, handle, tip; //static images
  //-----------------------------------
  Slider(PImage[] sliderImgs_, String sliderLabel, int x, int y, int sliderID) {
    ...
    posX = x;
    posY = y;
    id = sliderID; //each slider is given a unique number to keep from stepping on each other's toes
    setLimits(0.0, 1.0); //set min, max, and span
    val = 0.5; //default value
    label = sliderLabel + id; //default blank label
